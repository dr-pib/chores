...
- ~~**Completed truck check gets unchecked when secondary truck added via Edit Shift**~~ ✅ Fixed — the edit path now only deletes `status: 'pending'` Truck Checks and skips recreating any (unit, date) combo that already has a completed check (`completedTcKeys` set in `app/api/operations-logs/route.ts`). Completed checks and their credit/SW links stay intact across shift edits.
