...
- Daily Truck Check and NARC Box Check timing: due at shift start + 1 hour, but a late item should first show as overdue while still actionable. Only after the meaningful work window closes does forfeitable work become `missed`. Overdue and missed are different states and should be colored/listed/handled differently.
