...
- The supervisor+ nav keeps only daily-operational links inline (Setup, Chores, Roster, Dashboard); config/admin pages collapse into an `Admin` dropdown so the bar does not sprawl. The Admin button highlights when the active route is one of its items. Regular employees see only Setup/Chores/Roster. The mobile hamburger lists every link flat. Lives in `components/NavBar.tsx` (`primaryLinks` / `adminLinks`).
- Admin-dropdown menu labels (user-facing nav text only; routes and Prisma/model names are unchanged): `Performance Report` (`/report`), `Shift Templates` (`/shift-profiles`), `Employee Profiles` (`/employees`), `Chore Console` (`/chore-templates`), `Change Log` (`/change-log`), `To-Do` (`/todo`), `Dev` (Dom only). Note these nav labels intentionally differ from internal names — the model is still `ShiftProfile`, the config concept is still chore templates.
- `/todo` is a supervisor+ in-app To-Do/roadmap board (hand-maintained arrays in `app/todo/page.tsx`: "Recently shipped" and "Open / to review"). It is the human-readable summary for review at login; `AI_WORKPLAN.md` stays the detailed engineering source of truth. Update both when work lands.
- The nav bar uses the same dark green as the app background (`#0a140f`) but solid, with no dots (it is not a `.min-h-screen.bg-zinc-950` element, so the dotted CSS never applies). The Admin dropdown panel and mobile menu surface match.
...
- Deleting a shift (`DELETE /api/operations-logs/[id]`) purges the shift inside one transaction: chores, chore tasks, and bays cascade via the schema; `ChangeLog` rows tied to the shift / its chores / its deleted claimed work are removed; performance drops automatically because stats derive only from existing logs. Claimed `ScheduledWork` is split by status: **pending** claimed work reverts to unassigned (`claimed_by_log_id` set null) so still-needed asset work (e.g. an undone NARC Expires) keeps showing to supervisors; **completed/resolved** claimed work is deleted with the shift (completions/performance erased on purpose). Do not collapse this back to deleting all claimed SW — that would make still-needed pending work silently vanish.
- **TODO (maintainer-requested, not yet built):** when a shift is deleted, a **completed** NARC Expires must not just disappear — the underlying asset work is still owed once its completion is erased. Change the delete handler so completed claimed NARC Expires (and likely other completed persistent expires) revert to unassigned AND have their completion cleared (`status: 'pending'`, `completed_by_id`/`completed_at` null, `is_late_completion` false) so the box's NARC Expires shows as UNDONE / still needing to be done, instead of being deleted. See `/todo`.
...
- NARC box model — **built** (foundation complete):
  ...
  - NARC Expires is identified by its NARC box only, never by unit number. Display shows just the box letter, e.g. `NARC Expires Box C` (not `Box C Unit 4`). `ChoreItem` defaults `hideNarcUnit = true` so the unit chip is suppressed for NARC Expires on every screen. The supervisor overdue-expires ticker (`/api/alerts/overdue-expires`) lists NARC Expires by box letter for both the chore branch (from `operations_log.narc_box`) and the unassigned-ScheduledWork branch.
