...
- Deleting a shift (`DELETE /api/operations-logs/[id]`) is a **full purge**: chores, chore tasks, and bays cascade via the schema; the handler additionally deletes the shift's claimed `ScheduledWork` and any `ChangeLog` rows tied to the shift / its chores / its claimed work, inside one transaction. Performance drops automatically because stats derive only from existing completed logs. This is intentional ("at least for now" per maintainer): a deleted shift leaves no trace, and claimed asset work is deleted rather than reverted to unassigned. If asset accountability after deletion is later desired, revisit this handler.
